...
public interface TagihanDao extends PagingAndSortingRepository<Tagihan, String> {
    ...
    @Query(value = "select a.id,a.id_tagihan,angkatan,a.nim,a.nama,sum(a.tagihan) as tagihan,coalesce(sum(b.amount),0) as dibayar, sum(coalesce(a.tagihan,0))-sum(coalesce(b.amount,0)) as sisa, a.status, coalesce(nama_beasiswa, '-') from (select a.id as id, b.id as id_tagihan, a.angkatan as angkatan, a.nim as nim, a.nama as nama, a.status_aktif as status,b.nilai_tagihan as tagihan, c.id as id_beasiswa, c.nama_beasiswa as nama_beasiswa from mahasiswa as a inner join tagihan as b on a.id=b.id_mahasiswa left join beasiswa as c on a.id_beasiswa=c.id where b.status='AKTIF' and a.id_prodi=?1 and b.id_tahun_akademik=?2)a left join (select id_tagihan,sum(amount)as amount from pembayaran group by id_tagihan) as b on a.id_tagihan = b.id_tagihan left join mahasiswa_beasiswa as c on a.id_beasiswa=c.id_beasiswa group by a.nim", nativeQuery = true)
    List<Object[]> listTagihanPerMahasiswaByProdi(String idProdi, String idTahunAkademik);

    @Query(value = "select a.id,a.id_tagihan,a.prodi,a.nim,a.nama,sum(a.tagihan) as tagihan,coalesce(sum(b.amount),0) as dibayar, sum(coalesce(a.tagihan,0))-sum(coalesce(b.amount,0)) as sisa, a.status, coalesce(nama_beasiswa, '-') from (select a.id as id, b.id as id_tagihan, c.nama_prodi as prodi, a.nim as nim, a.nama as nama, a.status_aktif as status,b.nilai_tagihan as tagihan, d.id as id_beasiswa, d.nama_beasiswa as nama_beasiswa from mahasiswa as a inner join tagihan as b on a.id=b.id_mahasiswa inner join prodi as c on a.id_prodi=c.id left join beasiswa as d on a.id_beasiswa=d.id where b.status='AKTIF' and a.angkatan=?1 and b.id_tahun_akademik=?2)a left join (select id_tagihan,sum(amount)as amount from pembayaran group by id_tagihan) as b on a.id_tagihan = b.id_tagihan left join mahasiswa_beasiswa as c on a.id_beasiswa=c.id_beasiswa group by a.nim", nativeQuery = true)
    ...
    @Query(value = "select a.* from tagihan as a inner join mahasiswa as b on a.id_mahasiswa=b.id where a.status_tagihan='AKTIF' and lunas='0' and b.nim=?1", nativeQuery = true)
