...
public interface SeminarDao extends PagingAndSortingRepository<Seminar, String> {
    ...
    @Query("select s from Seminar s where s.status = 'APPROVED' and s.tahunAkademik = :tahun and (s.ketuaPenguji = :ketua or s.dosenPenguji = :dosen or s.note.dosen = :dosen1 or s.note.dosen2 = :dosen2) order by s.statusSempro desc ")
    List<Seminar> cariSeminar(@Param("ketua") Dosen ketua,@Param("dosen")Dosen dosen, @Param("dosen1") Dosen dosenNote,@Param("dosen2") Dosen dosenNote2,@Param("tahun") TahunAkademik tahunAkademik);

    @Query("select s from Seminar s where s.status = 'APPROVED' and s.note.mahasiswa.idProdi.idJenjang = :jenjang and s.tahunAkademik = :tahun and  s.note.dosen2 = :dosen order by s.statusSempro desc ")
    List<Seminar> cariSeminarPasca(@Param("dosen")Dosen dosen, @Param("jenjang") Jenjang jenjang,@Param("tahun") TahunAkademik tahunAkademik);
