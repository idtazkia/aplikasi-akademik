...
public interface RequestCicilanDao extends PagingAndSortingRepository<RequestCicilan, String> {
    ...
    @Query(value = "select tanggal_jatuh_tempo from request_cicilan where id_tagihan=?1 and status='AKTIF'", nativeQuery = true)
    List<Date> listOfDates(String idTagihan);
    ...
    @Query(value = "select a.nilai_tagihan-sum(coalesce(b.nilai_cicilan,0)) as sisa from tagihan as a inner join request_cicilan \n" +
            "as b on a.id=b.id_tagihan where a.status='AKTIF' and b.status='AKTIF' and a.id=?1", nativeQuery = true)
    BigDecimal sisaCicilan(String idTagihan);

    @Query(value = "select a.id, c.nama as nama, e.nama as tagihan, count(b.id) as cicilan from tagihan as a inner join mahasiswa as c on a.id_mahasiswa=c.id inner join nilai_jenis_tagihan as d\n" +
    ...
    RequestCicilan findByTagihanAndTanggalJatuhTempoNotInAndStatus(Tagihan tagihan, LocalDate tanggalJatuhTempo, StatusRecord statusRecord);
