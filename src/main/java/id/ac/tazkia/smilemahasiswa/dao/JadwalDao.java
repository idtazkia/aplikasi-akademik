...
public interface JadwalDao extends PagingAndSortingRepository<Jadwal, String> {
    ...
    @Query(value = "select a.*, coalesce(b.total,'-') as totalSks, coalesce(c.total_mahasiswa,'-') as jumlahMahasiswa from (select j.id, d.id as idDosen, k.id as idKelas, coalesce(r.nama_ruangan,'-') as Ruangan, coalesce(j.jam_mulai,'-') as jamMulai, coalesce(j.jam_selesai,'-') as jamSelesai, prod.nama_prodi as Prodi, m.kode_matakuliah as kodeMatakuliah, m.nama_matakuliah as namaMatakuliah, k.nama_kelas as namaKelas, kr.nama_karyawan as Dosen, coalesce(h.nama_hari,'-') as Hari from jadwal as j inner join matakuliah_kurikulum as mk on j.id_matakuliah_kurikulum=mk.id inner join matakuliah as m on mk.id_matakuliah=m.id inner join kelas as k on j.id_kelas=k.id inner join dosen as d on j.id_dosen_pengampu=d.id inner join karyawan as kr on d.id_karyawan=kr.id inner join ruangan as r on j.id_ruangan=r.id inner join prodi as prod on j.id_prodi=prod.id inner join hari as h on j.id_hari=h.id where j.status='AKTIF' and j.id_tahun_akademik=?1)a left join (select d.id, k.nama_karyawan as dosen, sum(mk.jumlah_sks) as total FROM jadwal as j inner join matakuliah_kurikulum as mk on j.id_matakuliah_kurikulum = mk.id inner join dosen as d on j.id_dosen_pengampu = d.id inner join karyawan as k on d.id_karyawan = k.id where id_tahun_akademik = ?1 and j.status = 'AKTIF' group by d.id)b on a.idDosen=b.id left join (select count(a.id) as total_mahasiswa,a.id_jadwal from krs_detail as a left join krs as b on a.id_krs = b.id where b.id_tahun_akademik=?1 and a.status='AKTIF' and b.status='AKTIF' group by id_jadwal)c on a.id=c.id_jadwal order by Prodi, namaKelas", nativeQuery = true)
