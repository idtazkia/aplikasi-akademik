...
public interface PraKrsSpDao extends PagingAndSortingRepository<PraKrsSp, String> {
    ...
    @Query(value = "select b.id, c.nama_matakuliah, coalesce(count(a.id_mahasiswa),0) as jumlah, a.status_approve as status from pra_krs_sp as a " +
            "inner join matakuliah_kurikulum as b on a.id_matakuliah_kurikulum=b.id " +
            "inner join matakuliah as c on b.id_matakuliah=c.id inner join tahun_akademik as d on a.id_tahun_akademik=d.id where prioritas=?1 and a.status='AKTIF' " +
            "and d.status!='NONAKTIF' and d.status!='HAPUS' group by c.kode_matakuliah", nativeQuery = true)
    List<Object[]> listKrsSp(String prioritas);
    ...
    List<PraKrsSp> findByStatusAndMatakuliahKurikulumAndPrioritas(StatusRecord statusRecord, MatakuliahKurikulum matakuliahKurikulum, String prioritas);
    ...
    List<PraKrsSp> findAllByStatusAndPrioritas(StatusRecord statusRecord, String prioritas);
