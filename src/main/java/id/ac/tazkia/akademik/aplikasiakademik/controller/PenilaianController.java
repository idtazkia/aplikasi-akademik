...
import id.ac.tazkia.akademik.aplikasiakademik.dto.NilaiTugasDao;
...
public class PenilaianController {
    ...
    public void listPenilaian(Model model,@PageableDefault(size = 5) Pageable page){
        List<PenilaianDto> penilaianDtos= new ArrayList<>();
        TahunAkademik tahunAkademik = tahunAkademikDao.findByStatus(StatusRecord.AKTIF);
        Page<Jadwal> jadwal =jadwalDao.cariDosen(StatusRecord.AKTIF,tahunAkademik,page);
        model.addAttribute("dosen",jadwal);
        model.addAttribute("jadwal", jadwalDao.findByStatusAndTahunAkademik(StatusRecord.AKTIF,tahunAkademik));
    }
    ...
    public String saveBobot(@ModelAttribute @Valid Jadwal jadwal){
    ...
    public void nilaiPenilaian(@RequestParam Jadwal jadwal, Model model){
        model.addAttribute("absensi", presensiDosenDao.findByStatusAndJadwal(StatusRecord.AKTIF,jadwal).size());
        model.addAttribute("jumlahMahasiswa", krsDetailDao.findByJadwalAndStatusOrderByMahasiswaNamaAsc(jadwal,StatusRecord.AKTIF).size());
    ...
    public void downloadExcel (@RequestParam(required = false) String id, HttpServletResponse response) throws IOException {
        ...
        staticColumn2.add("UTS");
        staticColumn2.add("UAS");
        ...
        for (KrsDetail kd : krsDetail) {
            ...
            for (BobotTugas bt : bobotTugas) {
                ...
                +"*"+bt.getBobot().toPlainString()+"/"+new Integer(100));
                kolom++;
            }

            for (String s : staticColumn2) {
